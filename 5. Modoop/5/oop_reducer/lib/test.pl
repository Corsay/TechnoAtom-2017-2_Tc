...
my $reducerMaxDiff2 = Local::Reducer::MaxDiff->new(
    top => 'received',
    bottom => 'sended',
    source => Local::Source::Text->new(text =>"sended:1024,received:2048\nsended:2048,received:10240\nsended:abc,received:10240"),
    row_class => 'Local::Row::Simple',
    initial_value => 0,
);
#p $reducerMaxDiff2; 
#p $reducerMaxDiff2->reduced();
#p $reducerMaxDiff2->reduce_all();
...
my $LRMMA1 = Local::Reducer::MinMaxAvg->new(
    field => 'price',
    source => Local::Source::Array->new(array => [
        '{"price": 1}',
        '{"price": 2}',
...
#p $LRMMA1;
my $try = $LRMMA1->reduce_n(3);
#p $try;
#p $try->get_min;#, 1, 'min';
#p $try->get_max;#, 3, 'max';
#p $try->get_avg;#, 2, 'avg';
$try = $LRMMA1->reduce_all();
#p $try->get_min;#, 1, 'min';
#p $try->get_max;#, 16, 'max';
#p $try->get_avg;#, 7, 'avg';

use Local::Source::FileHandle;
...
p $src;#, 'created source';
p $src->next;#, "test1", "first";
p $src->next;#, "test2", "second";
p $src->next;#, "test3", "third";
p $src->next;#, undef, "other 1";
p $src->next;#, undef, "other 2";
