...
#say GetColoredPatternStr("р Пер Тр ПАТЬр ",qr/$pattern/);
#say '';
...
foreach (@line) {
	...
	if ($check_sub->($elem, $pattern) != -1) {
		...
		unless ($flag_c) {
			...
			unless ($flag_v) {
				print $paint_sub->($elem, $pattern, $check_sub);
			}
			else {
				print "$elem";
			}
		...
		if ($flag_v) {
			$elem = $paint_sub->($elem, $pattern, $check_sub);	#Третий параметр для работы с опцией -F
		}
		# если установлена опция -n выводим с номером строки
		if ($flag_n) {
