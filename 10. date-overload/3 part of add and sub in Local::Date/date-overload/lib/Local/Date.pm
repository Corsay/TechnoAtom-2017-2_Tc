...
sub _add {
	...
	if ($date2->isa('Local::Date::Interval')) {	# `Local::Date::Interval`
		...
		#$date->{epoch} += $date2->duration();
		#$date->_get_data_comp();	# получаем компоненты даты (по GMT)
		return $date;
	}
...
sub _subtract {
	...
	if ($date2->isa('Local::Date::Interval')) {	# `Local::Date::Interval`
		...
		$date1->_get_data_comp();	# получаем компоненты даты (по GMT)
		return $date1;
	}
	elsif ($date2->isa('Local::Date')) {	# `Local::Date`
		# ToDo implement subtract Local::Date
	}
	elsif ($date2 =~ /^\d+$/) {	# Целое число
		$date1->{epoch} -= $date2;
		return $date1->{epoch};
