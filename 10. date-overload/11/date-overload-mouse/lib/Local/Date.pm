...
after seconds => sub {
	...
	my $min = int($sec / 60);
	$sec -= $min * 60;
	if ($sec < 0) {	# при вычитании времени
		$min--;
		$sec = 60 + $sec;
	}
...
after minutes => sub {
	...
	my $hours = int($min / 60);
	$min -= $hours * 60;
	if ($min < 0) {	# при вычитании времени
		$hours--;
		$min = 60 + $min;
	}
...
after hours => sub {
	...
	my $day = int($hours / 24);
	$hours -= $day * 24;
	if ($hours < 0) {	# при вычитании времени
		$day--;
		$hours = 24 + $hours;
	}
...
after month => sub {
	...
	$month -= $year * 12;
	if ($month < 0) {	# при вычитании времени
		$year--;
		$month = 12 + $month;
	}
