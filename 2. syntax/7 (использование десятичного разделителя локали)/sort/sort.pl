...
# ссылка на хеш зависящих от локали данных
my $locale_values = localeconv();
# десятичный разделитель локали $locale_values->{'decimal_point'};
my $dec_point = $locale_values->{'decimal_point'};
...
sub getDigSufOst {
	...
	foreach my $val (@letters) {
		...
		my $ord = ord($val);
		if ($sufdig == -2 and ($ord < 58 and $ord > 47 or $ord == ord($dec_point))) {
			# если цифра
			if ($ord != ord($dec_point)) {
				if ($f_point) {
