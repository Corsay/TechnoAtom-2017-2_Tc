...
post '/' => sub {
	...
	# выбираем всех пользователей которые по факту есть в базе и выдаем им права
	#$sth = database->prepare('SELECT login from user where login IN (?);');
	#for my $cur_user (@$accepted_users) {$cur_user = "'$cur_user'";}
	#$accepted_users = join ',', @$accepted_users;
	#$sth->execute($accepted_users);
	#my @accepted_users = $sth->fetchall_arrayref();
...
post '/login' => sub {
	...
	# ToDo решить проблему с utf символами (пароль на русском)
