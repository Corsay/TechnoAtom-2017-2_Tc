...
=head
					...
					if (my $pidP = open(READ_FROM_CHILD_TWO, '-|')) {
						if ($num != $#line) {
							print STDERR "in\n";
							close (IN_FROM_P);
							print OUT_TO_C $_ while (<READ_FROM_CHILD_TWO>);
							close (OUT_TO_C);
						}
						else {
							# иначе печатаем в родительский процесс
							print STDERR $_ while (<READ_FROM_CHILD_TWO>);
						}
						waitpid ($pidP, 0);
					} else {
						# второй дочерний
...
=head
					...
					if (my $pidP = fork()) {
						...
						if ($num) {	# если команда не первая
							close (OUT_TO_C);
							open (STDIN, '<&', 'IN_FROM_P') or die $!; # читаем IN_FROM_P(он же OUT_TO_P предыдущей команды) в STDIN текущей
							close (IN_FROM_P);
						}
						close (IN_FROM_C);
...
=cut
					...
					if (my $pidP = fork()) {
						...
						while (<IN_FROM_C>) {
							push @childSTDOUT, $_;
							kill "TERM", $pidP if (@childSTDOUT > $max_buff_size);
						}
						#print "killed?\n";
						close (IN_FROM_C);
						waitpid ($pidP, 0);
