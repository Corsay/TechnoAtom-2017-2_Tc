...
sub ps {
	...
	# выводим инфо строку 
	print {$wfh} "F S   UID   PID  PPID  C PRI  NI ADDR SZ WCHAN  TTY          TIME CMD\n";
	...
	while (my $fname = readdir $dh) {
		if ($fname =~ /\d+/) {
			...
			$proc{$fname}{PRI} = 80 + $proc{$fname}{NI};
			#p %proc;
			#print {$wfh} "F S   UID   PID  PPID  C PRI  NI ADDR SZ WCHAN  TTY          TIME CMD\n";
			my $line = sprintf("%1d %1s %5d %5d ","4",$proc{$fname}{S},$proc{$fname}{UID},$proc{$fname}{PID});
			$line .= sprintf("%5d %2d %3d %3d ",$proc{$fname}{PPID},"00",$proc{$fname}{PRI},$proc{$fname}{NI});
			$line .= sprintf("%1s %5d %6s %8s ","-",$proc{$fname}{SZ},$proc{$fname}{WCHAN},$proc{$fname}{TTY});
			$line .= sprintf("%8s %s","00:00:00",$proc{$fname}{CMD});
			print {$wfh} "$line\n";
			#last;
		}
	}
